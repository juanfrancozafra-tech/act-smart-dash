...
| `AtRiskAccountsTable` | Sortable account table with two variants. **`variant="hero"`** (the only variant rendered on the dashboard today): title *"Accounts Requiring Immediate Attention"*, primary-tinted border/ring, columns `Account · Health · Primary risk · Risk · Activation · ARR`. **`variant="default"`** (full list — kept for reuse on future surfaces, no longer mounted on the dashboard): columns `Account · Health · Primary risk · Risk · Invites · Last active · ARR`. **Every row is fully clickable** via `useNavigate` on the `<tr>` (with `role="link"`, `tabIndex={0}`, and Enter/Space keydown handling) and routes to `/accounts/$id`; the account-name link inside the row is kept for visual affordance and a11y. Owns sort persistence (`localStorage` key `retain:atrisk-sort`) and FLIP row animation. `activationStatus()` derives the color-coded onboarding badge (`<40%` danger, `<80%` warn, else ok). |
